...
Shader "Custom/RefractAndDivide" {

  Properties {

    _RefractDist( "RefractRayDistance",float) = .009
  ...
  SubShader {
    ...
    Pass {
      ...
      uniform float _ShatterDistance;
      
      VertexOut vert(VertexIn v) {
        
        VertexOut o;

        //v.position.xyz += v.normal * 0.5;
      ...
      fixed4 frag(VertexOut v) : COLOR {
        ...
        float2 texcoord = float2(1,1)-  sp.yx/sp.w;
      ...
      [maxvertexcount(3)]
        void geom(triangle VertexOut input[3], inout TriangleStream<VertexOut> OutputStream)
        {
            VertexOut test = (VertexOut)0;
            float3 normal = normalize(cross(input[1].mPos.xyz - input[0].mPos.xyz, input[2].mPos.xyz - input[0].mPos.xyz));
            for(int i = 0; i < 3; i++)
            {
                test.normal = normal;
                test.pos = input[i].pos;
                test.pos.xyz += normal * _ShatterDistance;
                test.uv = input[i].uv;
                test.mPos = input[i].mPos;
